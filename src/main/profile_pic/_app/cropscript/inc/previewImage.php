...
if((count($errors)==0)&&(file_exists($passedImage))) { 	// check if image exists + no errors found
	...
	$_image->newPath = ($use_absolute_path) ? $img_path : $_SERVER['DOCUMENT_ROOT'].$docRoot.$img_path; // set new path for temporary crop preview
	...
	copy($crop,$_image->newPath.$prefix.basename($crop));			// make a copy of the fullsize crop
