jQuery(window).load(function(){
	...
	var aspectRatio = (document.aspectRatio)? document.aspectRatio : 0.8;	// default min image height
	var setSelect = (document.setSelect) 	? eval(document.setSelect) : [ 50, 50, 150, 175 ];	// default min image height
	jQuery('#cropbox').Jcrop({
		...
		aspectRatio: aspectRatio					// crop box aspect ratio
	});
});
	...
	if(typeof(coords)=='object') {
		...
		jQuery.ajax({ 
			...
			success: function(ret){
				...
				// run on completetion
				if((ret.substr(0,4)=='FAIL')||(ret.substr(0,4)=='<br ')) {
					alert(ret.substr(5));
				} else {
